        ...
        const proxy = new Proxy(obj, {
            ...
            get(target, key) {
                ...
                if (Array.isArray(target) && key === "splice") {
                    return function (startIndex, deleteCount) {
                        const spliced = [];
                        const new_target = [];

                        for (let i = 0; i < target.length; i++) {
                            if (i >= startIndex && i < (startIndex + deleteCount)) {
                                spliced.push(target[i])
                                continue;
                            }
                            new_target.push(target[i]);
                        }

                        for (let i = 0; i < new_target.length; i++) {
                            target[i] = new_target[i];
                        }
                        for (let i = new_target.length; i < target.length; i++) {
                            delete target[i];
                        }

                        target.length = new_target.length;
